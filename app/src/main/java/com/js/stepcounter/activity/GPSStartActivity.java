...
import com.js.stepcounter.model.GpsModel;
import com.js.stepcounter.model.GpsTrackerModel;
import com.js.stepcounter.model.location;
import com.js.stepcounter.sevices.LocationBgService;
    ...
    private class MyReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals("GET_SIGNAL")) {
                TargetType = SensorService.IsTargetType;
//                mGpsModel = (GpsModel) intent.getExtras().getSerializable("gpsmodeldata");
                mGpsModel = SensorService.gpsModel;
...
//                Logger.e(locationArrayList.size());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (SensorService.IsNotiFlag) {
            SensorService.gpsModel = SensorService.gpsModel;
//            Logger.e("GSON activity" + new GsonBuilder().create().toJson(gpsModel));
            numStep = SensorService.gpsModel.getCurrStep();
            seconds = SensorService.gpsModel.getSecond();

            Calories = String.valueOf(CommanMethod.calculateCalories(numStep, userWeight, userHeight));
            Distance = String.format("%.2f", CommanMethod.calculateDistance(numStep));
            SensorService.IsNotiFlag = false;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.cvPause:
                SensorService.isGpsService = false;
                SensorService.IsTimerStart = false;
...
//                stopService(intent1);
    ...
    public void showDailog() {
        ...
        cvResume.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SensorService.isGpsService = true;
                SensorService.IsTimerStart = true;
